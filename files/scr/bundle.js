...
if(klass)$('#facebox .content').addClass(klass)$('#facebox .content').append(data)
...
init(settings)function clickHandler(){$.facebox.loading(true)
var klass=this.rel.match(/facebox\[?\.(\w+)\]?/)
if(klass)klass=klass[1]
fillFaceboxFromHref(this.href,klass)return false}return this.bind('click.facebox',clickHandler)}function init(settings){if($.facebox.settings.inited)return true
else $.facebox.settings.inited=true
$(document).trigger('init.facebox')makeCompatible()
...
image.onload=function(){$.facebox.reveal('<div class="image"><img src="'+image.src+'" /></div>',klass)}image.src=href}function fillFaceboxFromAjax(href,klass){$.get(href,function(data){$.facebox.reveal(data,klass)})}function skipOverlay(){return $.facebox.settings.overlay==false||$.facebox.settings.opacity===null}function showOverlay(){if(skipOverlay())return
...
action[0](action[1])}this.actions=[]}}Primer.Layer=function(){this.primer=null
this.children=[]
this.calls=[]
this.xVal=0
this.yVal=0
this.visibleVal=true
this.mouseoverVal=function(){}this.mouseoutVal=function(){}this.mouseWithin=false}Primer.Layer.prototype={bind:function(primer){this.primer=primer
...
case"stroke":this.context.stroke();break}}for(var i in this.children){this.children[i].draw()}this.context.restore()},ghost:function(e){if(!this.visible){return}this.context.save()
this.context.translate(this.x,this.y)
for(var i in this.calls){var call=this.calls[i]
...
case"moveTo":this.context.moveTo(call[1],call[2]);break
case"lineTo":this.context.lineTo(call[1],call[2]);break
case"fill":this.ghostFill(e);break}}for(var i in this.children){e.localX-=this.x
e.localY-=this.y
this.children[i].ghost(e)}this.context.restore()},ghostDetect:function(e){if(this.context.isPointInPath(e.localX-this.x,e.localY-this.y)){if(!this.mouseWithin){this.primer.actions.push([this.mouseoverVal,e])}this.mouseWithin=true}else{if(this.mouseWithin){this.primer.actions.push([this.mouseoutVal,e])}this.mouseWithin=false}},ghostFillRect:function(e,x,y,w,h){this.context.beginPath()
this.context.moveTo(x,y)
this.context.lineTo(x+w,y)
this.context.lineTo(x+w,y+h)
this.context.lineTo(x,y+h)
this.context.lineTo(x,y)
this.ghostDetect(e)},ghostFill:function(e){this.ghostDetect(e)}}})();(function($){$.fn.relatizeDate=function(){return $(this).each(function(){$(this).text($.relatizeDate(this))})}$.relatizeDate=function(element){return $.relatizeDate.timeAgoInWords(new Date($(element).text()))}$r=$.relatizeDate
...
Ready to proceed?"}}Function.prototype.delay=function(time){return setTimeout(this,time)}$.fn.scrollTo=function(el,speed){var target,container
if(typeof el=='number'||!el){speed=el
target=this
container='html,body'}else{target=el
container=this}var offset=$(target).offset().top-30
$(container).animate({scrollTop:offset},speed||1000)
return this}$.gitbox=function(url,klass){$.facebox(function(){$.get(url,function(data){$.facebox(data,'nopad')
$('#facebox .footer').hide()})})}$.fn.spin=function(){return this.after('<img src="/images/modules/ajax/indicator.gif" id="spinner"/>')}$.fn.stopSpin=function(){this.next().remove()
return this}$(function(){$('.jshide').hide()
...
if($.fn.facebox)$('a[rel*=facebox]').facebox()if($.fn.fancyZoom)$('a[rel*=fancyzoom]').fancyZoom({directory:'images/fancyzoom'})
if($.fn.truncate){$('.truncate').bind('truncate',function(){$(this).truncate(50,{chars:/.*/})}).trigger('truncate')}if($.fn.corner)$('.corner').corner()if($.fn.relatizeDate)$('.relatize').relatizeDate()$('a[href=#][alt^=""]').hover(function(){window.status=$(this).attr('alt')},function(){window.status=''})
...
$(".autocompleter").keypress(function(){$(this).removeClass('accept')})}$.userAutocomplete()
...
if(e.shiftKey){target=window.location.hash.replace(/-\d+/,'')+'-'+target.replace(/\D/g,'')}window.location.hash=target}else{target=window.location.hash}if(lines=target.match(/#?(?:L|-)(\d+)/g)){lines=$.map(lines,function(line){return parseInt(line.replace(/\D/g,''))})
if(lines.length==1)return $('#LC'+lines[0]).css('background-color','#ffc')
for(var i=lines[0];i<=lines[1];i++)$('#LC'+i).css('background-color','#ffc')
$('#LC'+lines[0]).scrollTo(1)}return false}GitHub.scrollToHilightedLine=function(){var lines,target=window.location.hash
if(lines=target.match(/^#?(?:L|-)(\d+)$/g)){lines=$.map(lines,function(line){return parseInt(line.replace(/\D/g,''))})
$('#LID'+lines[0]).scrollTo(1)}}$(function(){GitHub.scrollToHilightedLine()
...
position=position[1]}var target=$('#toc a:contains("'+hash.replace('#','')+'")')
...
GitHub.Commit.jumpToHashFile()}}}$(function(){GitHub.Commit.jumpToHashFile()
...
GitHub.Commit.hovered=null}}$('#files').mouseout(function(e){var bubble=$(e.target)
bubble=bubble.is('.bubble')?bubble:bubble.parent()
if(bubble.is(':not(.faux-bubble)'))clearHovered()})$('#files').mouseover(function(e){var bubble=$(e.target)
bubble=bubble.is('.bubble')?bubble:bubble.parent()
if(!bubble.is('.bubble'))return
GitHub.Commit.hovered=bubble
if(bubble.is('.empty'))GitHub.Commit.dumpEmptyClass.call(bubble)
if(bubble.is(':not(.faux-bubble)'))GitHub.Commit.highlightLine.call(bubble)})
$('#files').click(function(e){var bubble=$(e.target)
bubble=bubble.is('.bubble')?bubble:bubble.parent()
if(!bubble.is('.bubble'))return
bubble.data('clicked',true)GitHub.currentBubble=bubble
...
url+='?position='+pos}if(pos){var firstLine=parseInt(bubble.parents('tbody').find('.line_numbers:first > a:first').text())
url+='&line='+(firstLine+pos-1)}$.gitbox(url)})
...
var el=this.select(++this.current)if(el.offset().top-$(window).scrollTop()+50>$(window).height())el.scrollTo(200)}else{this.select(0)}},prev:function(){if(!this.current){this.elements.eq(0).removeClass('selected')
return this.current=null}var el=this.select(--this.current)
if(el.offset().top-$(window).scrollTop()<0)el.scrollTo(200)},link:function(key){if(GitHub.Commits.current===null)return false
window.location=GitHub.Commits.selected().find('[hotkey='+key+']').attr('href')}}$(function(){GitHub.Commits.elements=$('.commit')
...
$('#invitees').append(html)}$('#invitee_box > p').hide()}},'json')
$(this).hide()
$('#invitee_box').show()
return false})
...
return false})});$(function(){$('#downloads .delete').click(function(){if(confirm('Are you sure you want to delete this?')){$(this).hide().parents('form').append('deleting&hellip;').submit()}return false})})
GitHub.editableGenerator=function(options){return function(_,self){var defaults={id:'field',tooltip:'Click to edit!',indicator:'Saving...',data:function(data){return $(self).attr('data')||data},style:"display: inline",onblur:'submit',callback:function(){(function(){if($(self).attr('data'))$(self).attr('data',$(self).text())$(self).trigger('truncate').next().show()
$(self).trigger('afterSave.editableGenerator')}).delay(20)}}return $(this).editable($(this).attr('rel'),$.extend({},defaults,options))}}$(function(){$('.edit_link').click(function(){$(this).prev().trigger('click')
...
return false})
$('#repository_postreceive_url').bind('afterSave.editableGenerator',function(){if($('#repository_postreceive_url').text().slice(0,4)=='http')$('#run_postreceive_test').show()
else
...
var data={field:this.name,value:this.checked}$(self).siblings('.flash').remove()
...
$('.public_repo, .private_repo, .public_security, .private_security').toggle()if($('.repo').is('.public'))$('.repo').removeClass('public').addClass('private')else $('.repo').removeClass('private').addClass('public')
$.post(this.href,'')
return false})
...
$('.toggle_watch').toggle()$.post($(this).attr('href'),{data:{}})
return false})
...
return false})
if($('#donation_creation_in_progress').length>0){$('#donate_activate_toggle').html("<span style='color:green;'>We're creating your Pledgie account. We'll PM you when it's ready!</span>")}$('#pledgie_deactivate').click(function(){$('#paypal').val('')
return true})
...
$.userAutocomplete()})})
return false})
$('.repo_toggle').click(function(){var options={}options['field']=this.id
...
$.get(url,{'raw':'true'},function(data){div.html('<pre>'+data+'</pre>')})}return false})
...
if(!$.trim(user))return
var list=$('#pull_request .recipients ul')var recipients=list.find('li').map(function(){return $.trim($(this).text())})
...
recipient_controls.each(function(){if($(this).is(':checkbox')&&!$(this).attr('checked')||$(this).is(':text')&&$(this).val()=='')empty.push($(this))})
...
if(auto_response.length>=40)auto_response+='...'
...
return false})
$('#facebox #rubygem_install form').livequery('submit',function(){var path=location.pathname.split('/')
...
addDockJump("Popular Forked",'/popular/forked')}},addDockJump:function(name,path){window.fluid.addDockMenuItem(name,function(){window.location='http://github.com'+path})}}$(GitHub.Fluid.init);$(function(){var head_sha=$('#forkqueue #head-sha').text();$('#forkqueue .untested').each(function(i,dom){var sha=$(dom).attr('name')
...
rows.each(function(i,dom){var sha=$(dom).attr('ref');$(dom).parents('tr').children('.icons').html('ignoring...');$.post("forkqueue/ignore/"+sha,{});$(dom).parents('tr').fadeOut('normal',function(){$(this).remove()});})}else if(action=='apply'){var form=$(this).parents('form');form.submit();}$(this).children('.default').attr('selected',1);});var fork_queue_selection_log=[]
...
var jFrom=jSorted[0]
var jTo=jSorted[1]
for(var k=jFrom;k<jTo;k++){if(selectionType==true){$('#forkqueue input.r-'+i+'-'+k).attr("checked","true")}else{$('#forkqueue input.r-'+i+'-'+k).removeAttr("checked")}}}}fork_queue_selection_log.push($(this).attr("class"))})
...
$('.icons',next).html('<img src="/images/modules/dashboard/news/commit.png" alt="Applied" />')}applyNextPatch()})}else{$('#new-head-sha').attr('value',head_sha)
$('#finalize').show()}}$('#refresh-network-data').each(function(){$.post("network_meta",function(data){$('#fq-refresh').show()
...
count=0}else{$('.business .slide').animate({top:"-"+(count*75)+"px"},1500)}}setInterval(slider,3000)}setTimeout(start,1000)}});$(function(){$('.cancel').click(function(){window.location='/inbox'
return false})
...
if(i>6){color=[this.randColorValue(color[0]),this.randColorValue(color[1]),this.randColorValue(color[2])]}this.colors.push(color)
i+=1}},drawImpactGraph:function(){var streams={}var repo=$('#impact_graph').attr('rel')
var self=this
...
bucketHeight+=this.normalizeImpact(chunk[1])+this.chunkVerticalSpace}if(bucketHeight>height){height=bucketHeight}}$('#impact_graph div').remove()
...
for(var i=first+1;i<last;i++){if(!this.bucketHasAuthor(data.buckets[i],author)){data.buckets[i].i.push([author,0])}}}return data},bucketHasAuthor:function(bucket,author){for(var j=0;j<bucket.i.length;j++){if(bucket.i[j][0]==parseInt(author)){return true}}return false},findFirst:function(author,data){for(var i=0;i<data.buckets.length;i++){if(this.bucketHasAuthor(data.buckets[i],author)){return i}}},findLast:function(author,data){for(var i=data.buckets.length-1;i>=0;i--){if(this.bucketHasAuthor(data.buckets[i],author)){return i}}},colorFor:function(author){var color=this.colors[author]
...
if(newVal>255){newVal=255}if(newVal<0){newVal=0}return(newVal)},drawBucket:function(streams,bucket,i){var maxY=0
var self=this
$.each(bucket.i,function(j,chunk){var authorID=chunk[0]
var impact=self.normalizeImpact(chunk[1])
if(!streams[authorID]){streams[authorID]=new Array()}streams[authorID].push([i*100,maxY,50,impact,chunk[1]])
...
$('.activator').remove()
for(var author in streams){if(author!=topAuthor){this.drawStream(author,streams,ctx,true)}}if(topAuthor!=null){this.drawStream(topAuthor,streams,ctx,false)}},drawStream:function(author,streams,ctx,activator){ctx.fillStyle=this.colorFor(author)
chunks=streams[author]
for(var i=0;i<chunks.length;i++){var chunk=chunks[i]
ctx.fillRect(chunk[0],chunk[1],chunk[2],chunk[3])
if(activator){this.placeActivator(author,streams,ctx,chunk[0],chunk[1],chunk[2],chunk[3],chunk[4])}if(i!=0){ctx.beginPath()
...
ctx.fill()}previousChunk=chunk}},drawStats:function(author,streams){chunks=streams[author]
for(var i=0;i<chunks.length;i++){var chunk=chunks[i]
var impact=chunk[4]
...
if(y<25){id='selected_author_text'}else{id='author_text'}var styles=''
styles+='position: absolute;'
styles+='left: '+x+'px;'
styles+='top: '+y+'px;'
styles+='width: 920px;'
...
if(window.location.hash&&(sorting=window.location.hash.match(/sort=(\w+)/))){setTimeout(function(){$('#sort_by_'+sorting[1]).click()},100)}if(window.location.hash&&(issue=window.location.hash.match(/issue\/(\d+)/))){var comment,parts
if(parts=window.location.hash.match(/comment\/(\d+)/)){comment=parts[1]}if($('#issue_'+issue[1]).size()==0){var findURL=window.location.pathname.match(/^\/[^\/]+\/[^\/]+\/issues/)[0]+'/'
findURL=findURL+issue[1]+'/find'
if(comment){findURL=findURL+'?comment='+comment}window.location=findURL
return false}GitHub.Issues.showIssue(issue[1])
$('#issues .loading').hide()
$('#issues #issue_list').show()
if(comment){GitHub.Issues.adjustViewForComment(comment)}}else{$('#issues #issue_list .issue').show()
$('#issues .loading').hide()
$('#issues #issue_list').show()
GitHub.Issues.adjustViewForTarget()}GitHub.Issues.Dragger.updateHandles()
...
if(form.is(':not(.ajaxified)'))this.ajaxifyEditIssueForm(form)
...
issue.find('.edit_issue').hide()},showEditCommentForm:function(comment){if(comment.find('form').is(':not(.ajaxified)'))this.ajaxifyEditCommentForm(comment)
...
if(commentForm.find('textarea').val().match(/^\W*$/)){var status=commentForm.find('.status')
...
if(github_user!=null){if(GitHub.Issues.collab){active.find('.label .remove').show()}if(!active.hasClass('read')){active.addClass('read')
GitHub.Issues.decrementUnreadCount()
$.post(this.repoURL+issue+'/read',{})}}$('#issues .admin .back_link').show().parent().find('.selectors').hide()
...
$.post(this.repoURL+id+'/read',{})}GitHub.Issues.decrementUnreadCount()},markAsUnread:function(issue){var active=issue instanceof jQuery?issue:$('#issue_'+issue),id=active.attr('id').split('_')[1]
if(active.hasClass('read')){active.removeClass('read')
$.post(this.repoURL+id+'/unread',{})}GitHub.Issues.incrementUnreadCount()},changeUnreadCount:function(delta){var menuText=$('#unread_count').text()
var matches=menuText.match(/\((\d+)\)/)
if(matches){var newCount=parseInt(matches[1])+delta}else{var newCount=delta<0?0:1}if(newCount==0){$('#unread_count').text('')}else{$('#unread_count').text('('+newCount+')')}},decrementUnreadCount:function(){this.changeUnreadCount(-1)},incrementUnreadCount:function(){this.changeUnreadCount(1)},changeOpenCount:function(delta){var tabText=$('#repo_menu li.active a').text()
...
GitHub.Issues.hideIssue()}var issueDiv=$('#issue_'+issue)
if(GitHub.Issues.target[0].id==issueDiv[0].id)GitHub.Issues.targetNext()
issueDiv.remove()
...
$.post(GitHub.Issues.repoURL+issue+'/close',{})},moveIssueToTop:function(issueID){var issue=$('#issue_'+issueID),dragger={target:issue}GitHub.Issues.Dragger.startDrag(dragger)
issue.remove()
...
return false}if(target.is(':input')||target.is('a:not(.issue_title)'))return true
...
var inboxURL=$('#open_issues_link').attr('href')if(window.location.pathname==inboxURL){$('.back_link').click()
...
box.change()},get target(){return $('.target')},focusIssuesSearch:function(){$('#issues .searchbar').focus()},showHotkeyHelp:function(){if(GitHub.Issues.keyboardShortcuts)return $.facebox(GitHub.Issues.keyboardShortcuts)$.facebox(function(){$.get('/javascripts/github.issues.js',function(data){var lines=[],description='',keys=data.replace(/[\s\S]*hotkeys\({([\s\S]+?)}\)[\s\S]*/mg,'$1')
$.each(keys.split("\n"),function(i,line){line=line.replace(/[\':,]/g,'')
line=line.replace('GitHub.Issues.','')
line=$.trim(line)
if(!line)return
line=line.split(' ')description=line[1].replace(/([A-Z])/g,' $1').toLowerCase()
...
if(color=labels['label'+label_id]){GitHub.Issues.Labels.setLabelColors(label,color)}if(GitHub.Issues.active!=null){$('#issue_'+issue+' .label .remove').show()}updated.push(issue)}}if(updated.length>0){$.post($('#new_label').attr('action')+'/'+label_id+'/append',{issues:updated.join(',')})}},selected:function(){var issues=[]
if(GitHub.Issues.active!=null){issues=[''+GitHub.Issues.active]}else{issues=$('#issues .list').find('input:checked').serializeArray().map(function(issue){return issue.value})}return issues},set listHash(newListHash){GitHub.Issues.currentListHash=newListHash
...
return issue[0]}}GitHub.Issues.Issue=function(id,priority,votes,updated){this.id=id
this.priority=priority
this.updated=updated
this._votes=votes
GitHub.Issues.list.push(this)}GitHub.Issues.Issue.prototype={get element(){return $('#issue_'+this.id)},get votes(){return this._votes},set votes(vote){vote=parseInt(vote)
...
if(up){voteLink.removeClass('enabled').find('img').attr('src','/images/modules/issues/upvote_disabled.png')}else{voteLink.addClass('enabled').find('img').attr('src','/images/modules/issues/upvote.png')}$.post(GitHub.Issues.repoURL+this.id+url,{})
return vote}}GitHub.Issues.Dragger={dragging:0,startingPosition:-1,sortDrag:function(e,ui){if(this.dragging<5){$(e.target).parents('.issue').addClass('floating')
this.dragging+=1}},startDrag:function(e){if(!/labels/.test(location.pathname))return
var target=$(e.target),actor=(target.is('li')?target:target.parents('li')),issues=$.makeArray($('#issue_list li'))this.startingPosition=$.inArray(actor[0],issues)},stopDrag:function(e){var id,sorting=[]
...
params.actor=actor[0].id.split('_')[1]}else{var url=GitHub.Issues.repoURL+'sort',params={sorting:null}$('#issue_list>li').each(function(i){id=this.id.split('_')[1]
GitHub.Issues.find(id).priority=i
sorting.push(id)})
params.sorting=sorting.join(',')}GitHub.Issues.Dragger.updateHandles()
$.post(url,params)
this.dragging=0},updateHandles:function(){$('#issue_list .handles .top.handle').show()
$('#issue_list .handles .top.handle:first').hide()}}GitHub.Issues.Labels={init:function(){},sortedLabelInsert:function(name,id){if(id==undefined){id=''}else{id='label'+id}var url=GitHub.Issues.repoURL+'labels/'+name
var html=''
html+='<li>'
...
if(hsb.b<50){borderColor={h:hsb.h,s:hsb.s,b:(hsb.b*1.4)}}else{borderColor={h:hsb.h,s:hsb.s,b:(hsb.b*.8)}}var hoverColor=null
if(hsb.b<50){hoverColor={h:hsb.h,s:hsb.s,b:(hsb.b*1.8)}}else{hoverColor={h:hsb.h,s:hsb.s,b:(hsb.b*.4)}}label.css('backgroundColor',GitHub.Color.rgb2hex(rgb));var textColor='white'
if((hsb.b>60&&hsb.s<40)||(hsb.b>70&&hsb.h>30&&hsb.h<200)){textColor='black'}label.css('borderColor',GitHub.Color.hsb2hex(borderColor))
...
label.find('.labeli .remove').css('color',textColor)}}GitHub.Issues.Labels.Editor={chooser:function(){return $('.labels .list li .chooser')},isOpen:function(){return(this.chooser().length>0)},close:function(){this.chooser().remove()},currentId:function(){var idname=this.chooser().parent().find('.label.dropdown').attr('rel')
...
if(GitHub.Issues.active){var issue='<input type="hidden" name="issue" value="'+GitHub.Issues.active+'" />'}else{var issue=''}var form=$('<form method="post" action="'+url+'" style="display:none">'+issue+'<input type="hidden" name="color" value="'+color+'" /></form>')
...
this.close()},remove:function(){confirm("Are you sure?")
var id=this.currentId()
...
this.close()}}GitHub.Color={hex2rgb:function(hex){hex=hex.toLowerCase().replace(/#/,'')
var rgb={}if(hex.length==6){rgb.r=parseInt(hex.substr(0,2),16)
...
rgb.b=parseInt((hex.substr(2,2)+hex.substr(2,2)),16)}return rgb},rgb2hsb:function(rgb){var vHue,vSaturation,vBrightness
rgb.r=parseFloat(rgb.r)
rgb.g=parseFloat(rgb.g)
rgb.b=parseFloat(rgb.b)
var cmax=(rgb.r>rgb.g)?rgb.r:rgb.g
if(rgb.b>cmax){cmax=rgb.b}var cmin=(rgb.r<rgb.g)?rgb.r:rgb.g
if(rgb.b<cmin){cmin=rgb.b}vBrightness=cmax/255.0
if(cmax!=0){vSaturation=(cmax-cmin)/cmax}else{vSaturation=0}if(vSaturation==0){vHue=0}else{var redc=(cmax-rgb.r)/(cmax-cmin)
var greenc=(cmax-rgb.g)/(cmax-cmin)
var bluec=(cmax-rgb.b)/(cmax-cmin)
if(rgb.r==cmax){vHue=bluec-greenc}else if(rgb.g==cmax){vHue=2.0+redc-bluec}else{vHue=4.0+greenc-redc}vHue=vHue/6.0;if(vHue<0)vHue=vHue+1.0}return{h:Math.round(vHue*360),s:Math.round(vSaturation*100),b:Math.round(vBrightness*100)}},rgb2hex:function(rgb){return'rgb('+rgb.r+','+rgb.g+','+rgb.b+')'},hsb2hex:function(hsb){return this.rgb2hex(this.hsb2rgb(hsb))},hsb2rgb:function(hsb){var rgb={}var h=Math.round(hsb.h)
...
if(h==360)h=0
if(h<60){rgb.r=t1;rgb.b=t2;rgb.g=t2+t3}else if(h<120){rgb.g=t1;rgb.b=t2;rgb.r=t1-t3}else if(h<180){rgb.g=t1;rgb.r=t2;rgb.b=t2+t3}else if(h<240){rgb.b=t1;rgb.r=t2;rgb.g=t1-t3}else if(h<300){rgb.b=t1;rgb.g=t2;rgb.r=t2+t3}else if(h<360){rgb.r=t1;rgb.g=t2;rgb.b=t1-t3}else{rgb.r=0;rgb.g=0;rgb.b=0}}return{r:Math.round(rgb.r),g:Math.round(rgb.g),b:Math.round(rgb.b)}}}GitHub.Issues.Nav={currentHash:null,ignoreHashChange:false,interval:null,init:function(){this.currentHash=window.location.hash
...
return $(this).blur()}var issues=GitHub.Issues.selected()
if(issues.length==0){alert("Please select an issue first")
$(this).val('gh-actions')
return $(this).blur()}var human=$(this).parent().find('option:selected').text()
...
if(labelSelected){GitHub.Issues.addLabels(issues,label_id,human)}$(this).val('gh-actions')
return $(this).blur()})
...
var list=$('#issue_list')
sorter=sorter||function(a,b){return a[property]>b[property]?1:-1}var issues=GitHub.Issues.list.sort(sorter)
...
$('.loading').hide()
return false}$('body').click(function(){if(GitHub.Issues.Labels.Editor.isOpen()){GitHub.Issues.Labels.Editor.close()}})
...
if(labelColor==null){labelColor='#ededed'}chooser.find('.label').addClass(labelName)
...
return false})
$('#issues .list input[type=checkbox]').change(function(){if($(this).attr('checked'))$(this).parents('.issue').addClass('selected')
else
$(this).parents('.issue').removeClass('selected')})
var issues_selection_log=[]
var renumberCheckboxes=function(){$('#issues .display .list input[type=checkbox]').each(function(i,el){$(el).attr('rel','r-1-'+i)})}renumberCheckboxes()
...
checkbox.parents('li').removeClass("selected")}}}}issues_selection_log.push($(this).attr("rel"))})
...
issues_selection_log=[]
return false})
if(GitHub.Issues.collab){$('#issues .display .list').sortable({axis:'y',containment:'#issues',handle:'.drag.handle',cancel:'.open, .voting_box',update:renumberCheckboxes,sort:GitHub.Issues.Dragger.sortDrag,start:GitHub.Issues.Dragger.startDrag,stop:GitHub.Issues.Dragger.stopDrag})}$('#issues .list input[type=checkbox]').change()
...
$(this).parent().remove()
if(list.find('li').length==0){$('#keys .body .danger').show()}}return false})
...
$(self).toggle()}$(self).find('textarea').val('')
$(self).find(':text').val('')}else{$(self).append(key)}$(self).find(':submit').attr('disabled',false).stopSpin()})
return false})});$(function(){if($('#network .out_of_date').length>0){var up_to_date=function(){$('#network .out_of_date').addClass("up_to_date").text("This graph has new data available. Reload the page to see it!");};var repo=$('#network .out_of_date').attr('rel');var tid=[];var pinger=function(){$.getJSON("/"+repo+"/network_current",function(data){if(data.current){up_to_date();clearInterval(tid[0]);}})}tid.push(setInterval(pinger,5000));}});$(function(){$(".graph .bars").each(function(i){var graph=this
var callback=function(data){new ParticipationGraph(graph,data)}var datasource=$(this).attr("rel")
...
this.data=data
this.readData()
this.draw()}ParticipationGraph.prototype={readData:function(){var data_strings=this.data.split("\n")
...
for(var i=1;i<arr.length;i++){if(arr[i]>max){max=arr[i]}}return max},integerize:function(arr){var out=new Array()
for(var i=0;i<arr.length;i++){out.push(parseInt(arr[i]))}return out},base64ByteToInt:function(byte){var chars="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!-"
...
arr.push(num)}}return arr},draw:function(){var max=this.max(this.allCommits)
var scale
if(max>=20){scale=19.0/max}else{scale=1}var allCommitsLayer=new Primer.Layer()
...
allCommitsLayer.addChild(square)}var ownerCommitsLayer=new Primer.Layer()
...
square.fillRect(i*(this.BAR_WIDTH+1),20-d,this.BAR_WIDTH,d)
ownerCommitsLayer.addChild(square)}this.primer.addChild(allCommitsLayer)
this.primer.addChild(ownerCommitsLayer)}};$(function(){$('#signup_form').submit(function(){$('#signup_button').attr('disabled',true).val('Creating your GitHub account...')})});GitHub.CachedCommitDataPoller=function(){this.startTimer()}GitHub.CachedCommitDataPoller.prototype={startTimer:function(){var poller=this
this.wait=this.wait?this.wait*1.5:2000
if(this.timer){this.clearTimer()
this.poll()}this.timer=setInterval(function(){poller.startTimer()},poller.wait)},poll:function(){var sha,commit,msg,user,poller=this,path=location.pathname,url=location.pathname.replace(/tree\/.+/,'cache/commits/')+GitHub.currentTreeSHA+'?path='+GitHub.currentPath+'&commit_sha='+GitHub.commitSHA,commitURL=location.pathname.replace(/tree\/.+/,'commit/')
$.getJSON(url,function(data){if(!data)return
poller.clearTimer()$('#browser tr').each(function(){if((sha=$(this).find('.content a').attr('id'))&&data[sha]){$(this).find('.age').html('<span class="drelatize">'+data[sha].date+'</span>')
msg=$(this).find('.message')
msg.html(data[sha].message)
if(msg.html().length>50)msg.html(msg.html().slice(0,47)+'...')
...
this.timer=null}}$(function(){$('#file-edit-link').click(function(){$('#readme').hide()
...
clearInterval(tid[0])}})}tid.push(setInterval(pinger,1000));})
...
return false})
var dim=function(){$(this).hide().prev().show()}$('#private-clone-url > :input').mouseout(dim).blur(dim)
...
$(function(){GitHub.UFO={drawFont:function(){var canvas=document.getElementById('ufo');var ctx=canvas.getContext('2d');for(var i=0;i<glifs.length;i++){ctx.save();var x=(i%9)*100;var y=Math.floor(i/9)*100;ctx.translate(x+10,y+80);ctx.scale(.1,-.1);var glif=new GitHub.UFO.Glif(ctx,glifs[i]);glif.draw();ctx.restore();}}}GitHub.UFO.Glif=function(ctx,contours){this.ctx=ctx;this.contours=contours;}GitHub.UFO.Glif.prototype={draw:function(){this.ctx.beginPath();for(var i=0;i<this.contours.length;i++){this.drawContour(this.contours[i]);}this.ctx.fillStyle='black';this.ctx.fill();},drawContour:function(vertices){for(var i=0;i<vertices.length;i++){if(i==0){this.moveVertex(vertices[i]);}else{this.drawVertex(vertices[i]);}}this.drawVertex(vertices[0]);},moveVertex:function(v){this.ctx.moveTo(v[0],v[1]);},drawVertex:function(v){if(v.length==2){this.ctx.lineTo(v[0],v[1]);}else if(v.length==4){this.ctx.quadraticCurveTo(v[2],v[3],v[0],v[1]);}else if(v.length==6){this.ctx.bezierCurveTo(v[2],v[3],v[4],v[5],v[0],v[1]);}}}if($('#ufo').length>0){GitHub.UFO.drawFont();}$('.glif_diff').each(function(el){var sha=$(this).attr('rel');var ctx=this.getContext('2d');var data=eval('glif_'+sha);var glif=new GitHub.UFO.Glif(ctx,data);ctx.translate(0,240);ctx.scale(.333,-.333);glif.draw();});});$(function(){$('a.follow').click(function(){$.post(this.href,{})
...
$('.user_toggle').click(function(){var options={}options[this.name]=this.checked?'1':'0'
options['_method']='put'
...
return false}$.post($(this).attr('href'),{email:$(this).prev().text()})
$(this).parent().remove()
return false})
...
return false})}$('#change_plan_toggle').click(function(){$('.plan_box').hide()
$('.user').hide()
$('#change_plan').show()
return false})
...
return false}$('.user').show()
$('strong.plan_pricing').hide()
$('#change_plan').hide()
if(plan=='coupon'){$('#coupon_box').show()
return false}$('#update_plan_status').html("<h2>You're "+direction+' to the '+human_plan+' Plan</h2>').show()
$('#'+plan+'_cost').show()
$('#update_cc').show()
if(!$('#can_update_cc').is('.dont_show'))$('#can_update_cc').show()return false})
